...
def Create_pipe(self):
    try:
        ...
        with self.config.connection.cursor() as cursor:
            ...
            last_oddo = 0
            pipes = []
            config_universal.print_with_time("Generating Pipe")
            last_weld = f_data[-1]
            # print(f_data)
            for row in f_data:
            ...
            config_universal.print_with_time("Pipe Generation completed")
            config_universal.print_with_time("process end at : ")
            QMessageBox.information(
                self,
                "Success",
                "Pipes created successfully and stored in db."
            )
