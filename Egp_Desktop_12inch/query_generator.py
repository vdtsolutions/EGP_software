...
def get_pipe(lower_sensitivity,upper_sensitivity,runid, start_index, end_index,pipe_id):
    ...
    with connection.cursor() as cursor:
        query = "SELECT proximity1, proximity2, proximity3, proximity4, proximity5, proximity6, proximity7, proximity8,proximity9, proximity10, proximity11, proximity12, proximity13, proximity14, proximity15,proximity16, proximity17, proximity18, proximity19, proximity20, proximity21, proximity22, proximity23, proximity24 FROM base_value where pipe_id={}".format(pipe_id)
    ...
    base_values.append(float(config["proximity19"]))

    base_values.append(float(config["proximity20"]))

    base_values.append(float(config["proximity21"]))

    base_values.append(float(config["proximity22"]))

    base_values.append(float(config["proximity23"]))

    base_values.append(float(config["proximity24"]))
